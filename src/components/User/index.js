...
export default function User(){
    ...
    let [{UserName,activities,groups,loaded,loans},setStates] = useState(1,2,false);
    useEffect(()=>{
    ...
    return(
        <div className="User">
            <h1 className="header">{loaded?UserName.toUpperCase():"loading..."}</h1>
            <form className="Search Bar">
                <label>Search Group: </label>
                <input type="text" placeholder="Group Name" onChange={handleChange}/>
                <Link to={{pathname:'/search',query:key}}><button>Search</button></Link>
            </form>
...
async function getStates(id){
    let info=await axios.get("http://localhost:8888/get_info?user_id="+String(id));
    let acts=await axios.get("http://localhost:8888/user/ra?user_id="+String(id));
    let grps=await axios.get("http://localhost:8888/user/select_group?user_id="+String(id));
    let lons=await axios.get("http://localhost:8888/user/status_category?user_id="+String(id));
